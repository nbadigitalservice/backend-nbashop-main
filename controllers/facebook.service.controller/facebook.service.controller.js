  ...
  try {
    ...
    upload(req, res, async function (err) {
      ...
      if (!req.files) {
        ...
        const facebookservicepackage = new FacebookPackage(data);
        facebookservicepackage.save(error => {
          if (error) {
...
module.exports.GetAll = async (req, res) => {
  try {
    const facebookservice = await FacebookPackage.find();
    return res.status(200).send({ status: true, message: 'ดึงข้อมูลสำเร็จ', data: facebookservice })
...
module.exports.GetById = async (req, res) => {
  try {
    const facebookservice = await FacebookPackage.findById(req.params.id);
    if (!facebookservice) {
...
module.exports.update = async (req, res) => {
  try {
    ...
    const packageUpdate = await FacebookPackage.findById(id)
    ...
    upload(req, res, async function (err) {
      ...
      if (!req.files) {
        ...
        FacebookPackage.findByIdAndUpdate(id, data, { returnDocument: 'after' }, (err, result) => {
          if (err) {
...
module.exports.delete = async (req, res) => {
  try {
    const id = req.params.id;
    FacebookPackage.findByIdAndDelete(id, { returnOriginal: true }, (error, result) => {
      if (error) {
