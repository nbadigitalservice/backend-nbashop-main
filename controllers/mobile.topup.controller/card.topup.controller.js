  ...
  try {
    ...
    if (data) {
      if (data.data.error_code === "E00") {
        ...
        let percent = 3;
        let profit_shop = 3;
        let profit_nba = 2;  //กำไร nba ที่จะเอา กรณีมีค่า charge มากกว่า 0 ขึ้นไป
        let cost = 0;
        let charge = 0;
        //หาต้นทุน
        if(parseFloat(data.data.charge.replace(/,/g, ""))>0){
            cost = (price+parseFloat(data.data.charge.replace(/,/g, ""))) + profit_nba
            charge = parseFloat(data.data.charge.replace(/,/g, ""))+ profit_nba + profit_shop
        }else{
            profit_shop = (price * (percent - nba)) / 100;
            profit_nba = (price * nba) / 100;
            cost = price - profit_shop;
            charge = 0;
        }
