...
module.exports.create = async (req, res) => {
    try {
        ...
        upload(req, res, async function (err) {
            ...
            if (!req.files) {
                res.status(500).send({ message: "มีบางอย่างผิดพลาด", data: 'No Request, Files', });
            } else {
                ...
                const Detachpackage = new DetachPackage(data);
                Detachpackage.save(error => {
                    if (error) {
                        res.status(403).send({ status: false, message: 'ไม่สารมารบันทึกได้', data: error });
                    } else {
...
module.exports.delete = async (req, res) => {
    try {
        const id = req.params.id;
        DetachPackage.findByIdAndDelete(id, { returnOriginal: true }, (err, result) => {
            if (err) {
