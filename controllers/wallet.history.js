...
module.exports.GetAll = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({status: true, message: "ดึงข้อมูลสำเร็จ", data: wallethistory});
  } catch (error) {
    console.error(error);
    return res
      .status(500)
  ...
  try {
    ...
    wallethistory.save((error) => {
      if (error) {
        res
          .status(403)
          .send({status: false, message: "ไม่สามารถบันทึกได้", data: error});
      } else {
        res
          .status(200)
          .send({status: true, message: "บันทึกสำเร็จ", data: wallethistory});
      }
    });
    //end
  } catch (error) {
