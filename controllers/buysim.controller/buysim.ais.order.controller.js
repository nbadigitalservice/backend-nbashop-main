const { BuysimAISPackage } = require('../../models/buysim.model/buysim.ais.package.model')
...
module.exports.order = async (req, res) => {
    try {
        const buysimaispackage = await BuysimAISPackage.findOne({ _id: req.body.product_detail[0].packageid });
        if (buysimaispackage) {
                ...
                if (err) {
                    jwt.verify(token, process.env.API_PARTNER_KEY, async (arr, partner_decoded) => {
                        if (err) {
                            ...
                            const totalprice = buysimaispackage.price * req.body.product_detail[0].quantity
                            const change = req.body.moneyreceive - totalprice
                            const totalplateformprofit = buysimaispackage.plateformprofit * req.body.product_detail[0].quantity
                            ...
                            let data = {
                                ...
                                product_detail: [{
                                    packageid: buysimaispackage._id,
                                    packagename: buysimaispackage.name,
                                    packagedetail: buysimaispackage.detail,
                                    quantity: req.body.product_detail[0].quantity,
                                    price: buysimaispackage.price,
                                }],
                                paymenttype: req.body.paymenttype,
                                moneyreceive: req.body.moneyreceive,
                                totalCost: buysimaispackage.cost + buysimaispackage.nbaprofit,
            ...
            `
                    ...
                    if (!checkuser) {
                        ...
                        if (!findshop) {
                            ...
                            if (partner.partner_wallet < buysimaispackage.price) {
                                ...
                                for (let item of req.body.product_detail) {
                                    const container = await BuysimAISPackage.findOne({ _id: item.packageid });
                                    if (container) {
                                        orders.push({
