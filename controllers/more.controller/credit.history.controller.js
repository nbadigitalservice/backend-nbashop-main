const {
  CreditHistory,
  validate,
} = require("../../models/more.model/credit.history.model");
  ...
  try {
    ...
    if (req.body.type === "เครดิตเข้า" || req.body.type === "เครดิตออก") {
      ...
      if (credit) {
        return res
          .status(201)
          .send({status: true, message: "สร้างรายการเรียบร้อย"});
      } else {
        return res
          .status(400)
      ...
      return res
        ...
        .send({
          status: false,
          message: "ประเภทจะต้องเป็น เครดิตเข้า หรือ เครดิตออก เท่านั้น",
        });
    }
  } catch (err) {
    console.log(err);
