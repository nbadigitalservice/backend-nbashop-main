    ...
    if (!checkuser) {
      ...
      if (!findshop) {
        ...
        for (let item of req.body.product_detail) {
          const websitepackage = await WebsitePackageModel.findOne({
            _id: item.packageid,
          });
          const total = websitepackage.price;
          product_price += total;
        }
        if (partner.partner_wallet < product_price) {
          ...
          for (let item of req.body.product_detail) {
            const container = await WebsitePackageModel.findOne({
              _id: item.packageid,
            });
            if (container) {
              const productgraphic = await WebsitePackageModel.findOne({
                _id: container._id,
              });
              if (productgraphic) {
