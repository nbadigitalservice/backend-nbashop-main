const { RegsimTRUEPackage } = require('../../models/regsim.model/regsim.true.model')
...
module.exports.order = async (req, res) => {
    try {
        const regsimtruepackage = await RegsimTRUEPackage.findOne({ _id: req.body.product_detail[0].packageid });
        if (regsimtruepackage) {
                ...
                if (err) {
                    jwt.verify(token, process.env.API_PARTNER_KEY, async (arr, partner_decoded) => {
                        if (err) {
                            ...
                            const totalprice = regsimtruepackage.price * req.body.product_detail[0].quantity
                            const change = req.body.moneyreceive - totalprice
                            const totalplateformprofit = regsimtruepackage.plateformprofit * req.body.product_detail[0].quantity
                            ...
                            let data = {
                                ...
                                servicename: 'regsim Service',
                                shopid: req.body.shopid,
                                shop_partner_type: req.body.shop_partner_type,
                                branch_name: req.body.branch_name,
                                branch_id: req.body.branch_id,
                                product_detail: [{
                                    packageid: regsimtruepackage._id,
                                    packagename: regsimtruepackage.name,
                                    packagedetail: regsimtruepackage.detail,
                                    quantity: req.body.product_detail[0].quantity,
                                    price: regsimtruepackage.price,
                                }],
                                paymenttype: req.body.paymenttype,
                                moneyreceive: req.body.moneyreceive,
                                totalCost: regsimtruepackage.cost + regsimtruepackage.nbaprofit,
                                    ...
                                    if (data) {
                                        ...
                                        commission.save((error, data) => {
                                            ...
                                            const wallethistory = {
                                                ...
                                                partner_id: 'platform',
                                                orderid: order._id,
                                                name: `รายการสั่งซื้อ Regsim Service ใบเสร็จเลขที่ ${order.receiptnumber}`,
                                                type: 'เงินออก',
            ...
            `
                    ...
                    if (!checkuser) {
                        ...
                        if (!findshop) {
                            ...
                            if (partner.partner_wallet < regsimtruepackage.price) {
                                ...
                                for (let item of req.body.product_detail) {
                                    const container = await regsimtruepackage.findOne({ _id: item.packageid });
                                    if (container) {
                                        orders.push({
                                        ...
                                        if (data) {
                                            ...
                                            commission.save((error, data) => {
                                                ...
                                                const wallethistory = {
                                                    ...
                                                    orderid: findorderid._id,
                                                    name: `รายการสั่งซื้อ Regsim Service ใบเสร็จเลขที่ ${findorderid.receiptnumber}`,
                                                    type: 'เงินออก',
