const { RegsimAISPackage } = require('../../models/regsim.model/regsim.ais.model')
...
module.exports.order = async (req, res) => {
    try {
        const regsimaispackage = await RegsimAISPackage.findOne({ _id: req.body.product_detail[0].packageid });
        if (regsimaispackage) {
                ...
                if (err) {
                    jwt.verify(token, process.env.API_PARTNER_KEY, async (arr, partner_decoded) => {
                        if (err) {
                            ...
                            const totalprice = regsimaispackage.price * req.body.product_detail[0].quantity
                            const change = req.body.moneyreceive - totalprice
                            const totalplateformprofit = regsimaispackage.plateformprofit * req.body.product_detail[0].quantity
                            ...
                            let data = {
                                ...
                                product_detail: [{
                                    packageid: regsimaispackage._id,
                                    packagename: regsimaispackage.name,
                                    packagedetail: regsimaispackage.detail,
                                    quantity: req.body.product_detail[0].quantity,
                                    price: regsimaispackage.price,
                                }],
                                paymenttype: req.body.paymenttype,
                                moneyreceive: req.body.moneyreceive,
                                totalCost: regsimaispackage.cost + regsimaispackage.nbaprofit,
            ...
            `
                    ...
                    if (!checkuser) {
                        ...
                        if (!findshop) {
                            ...
                            if (partner.partner_wallet < regsimaispackage.price) {
                                ...
                                for (let item of req.body.product_detail) {
                                    const container = await regsimaispackage.findOne({ _id: item.packageid });
                                    if (container) {
                                        orders.push({
