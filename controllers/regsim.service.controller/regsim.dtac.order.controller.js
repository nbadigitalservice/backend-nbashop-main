const { RegsimDTACPackage } = require('../../models/regsim.model/regsim.dtac.model')
...
module.exports.order = async (req, res) => {
    try {
        const regsimdtacpackage = await RegsimDTACPackage.findOne({ _id: req.body.product_detail[0].packageid });
        if (regsimdtacpackage) {
                ...
                if (err) {
                    jwt.verify(token, process.env.API_PARTNER_KEY, async (arr, partner_decoded) => {
                        if (err) {
                            ...
                            const totalprice = regsimdtacpackage.price * req.body.product_detail[0].quantity
                            const change = req.body.moneyreceive - totalprice
                            const totalplateformprofit = regsimdtacpackage.plateformprofit * req.body.product_detail[0].quantity
                            ...
                            let data = {
                                ...
                                product_detail: [{
                                    packageid: regsimdtacpackage._id,
                                    packagename: regsimdtacpackage.name,
                                    packagedetail: regsimdtacpackage.detail,
                                    quantity: req.body.product_detail[0].quantity,
                                    price: regsimdtacpackage.price,
                                }],
                                paymenttype: req.body.paymenttype,
                                moneyreceive: req.body.moneyreceive,
                                totalCost: regsimdtacpackage.cost + regsimdtacpackage.nbaprofit,
            ...
            `
                    ...
                    if (!checkuser) {
                        ...
                        if (!findshop) {
                            ...
                            if (partner.partner_wallet < regsimdtacpackage.price) {
                                ...
                                for (let item of req.body.product_detail) {
                                    const container = await regsimdtacpackage.findOne({ _id: item.packageid });
                                    if (container) {
                                        orders.push({
