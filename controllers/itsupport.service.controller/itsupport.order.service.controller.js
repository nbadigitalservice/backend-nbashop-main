    ...
    if (!checkuser) {
      ...
      if (!findshop) {
        ...
        for (let item of req.body.product_detail) {
          const ITpackage = await ItsupportPackage.findOne({
            _id: item.packageid,
          });
          const total = ITpackage.price;
          product_price += total;
        }
        if (partner.partner_wallet < product_price) {
          ...
          for (let item of req.body.product_detail) {
            const container = await ItsupportPackage.findOne({
              _id: item.packageid,
            });
            if (container) {
              const productgraphic = await ItsupportPackage.findOne({
                _id: container._id,
              });
              if (productgraphic) {
