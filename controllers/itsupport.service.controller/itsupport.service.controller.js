const { ItsupportPackage, validate } = require('../../models/itsupport.model/itsupport.package.model')
const fs = require('fs')
...
module.exports.create = async (req, res) => {
    try {
      ...
      if (!req.files) {
        ...
        const itsupportservicepackage = new ItsupportPackage(data);
        itsupportservicepackage.save(error =>{
            if(error){
...
module.exports.GetAll = async (req,res) =>{
    try {
      const itsupportservice = await ItsupportPackage.find();
      return res.status(200).send({status:true,message:'ดึงข้อมูลสำเร็จ',data:itsupportservice})
...
module.exports.GetById = async (req,res) => {
    try {
      const itsupportservice = await ItsupportPackage.findById(req.params.id);
      if(!itsupportservice){
...
module.exports.update = async (req,res) => {
    try {
        ...
        const packageUpdate = await ItsupportPackage.findById(id)
      ...
      if (!req.files) {
        ...
        ItsupportPackage.findByIdAndUpdate(id,data,{returnDocument:'after'},(err,result)=>{
          if(err){
...
module.exports.delete = async (req,res) => {
    try {
      const id = req.params.id;
      ItsupportPackage.findByIdAndDelete(id,{returnOriginal:true},(error,result)=>{
        if(error){
...
async function uploadFileCreate(req, res, { i, reqFiles }) {
    ...
    let fileMetaData = {
      ...
      parents: [process.env.GOOGLE_DRIVE_IMAGE_FACEBOOK_SERVICE],
    };
    let media = {
