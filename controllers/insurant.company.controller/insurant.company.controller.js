const { InsuranceCompanyModel } = require('../../models/insurant.company.model/insurant.company.model')
const fs = require('fs')
    ...
    try {
        ...
        upload(req, res, async function (err) {
            ...
            if (!req.files) {
                ...
                const insurancecompany = new InsuranceCompanyModel(data);
                insurancecompany.save(error => {
                    if (error) {
...
module.exports.GetAll = async (req, res) => {
    try {
        const insurancecompany = await InsuranceCompanyModel.find();
        return res.status(200).send({ status: true, message: 'ดึงข้อมูลสำเร็จ', data: insurancecompany })
...
module.exports.GetById = async (req, res) => {
    try {
        const insurancecompany = await InsuranceCompanyModel.findById(req.params.id);
        if (!insurancecompany) {
...
module.exports.update = async (req, res) => {
    try {
        ...
        const companyUpdate = await InsuranceCompanyModel.findById(id)

        let upload = multer({ storage: storage }).array("imgCollection", 20);
        upload(req, res, async function (err) {

            const name = req.body.name ? req.body.name : companyUpdate.name
            const address = req.body.address ? req.body.address : companyUpdate.address
            ...
            if (!req.files) {
                ...
                InsuranceCompanyModel.findByIdAndUpdate(id, data, { returnDocument: 'after' }, (err, result) => {
                    if (err) {
...
module.exports.delete = async (req, res) => {
    try {
        const id = req.params.id;
        InsuranceCompanyModel.findByIdAndDelete(id, { returnOriginal: true }, (error, result) => {
            if (error) {
...
async function uploadFileCreate(req, res, { i, reqFiles }) {
    ...
    let fileMetaData = {
        ...
        parents: [process.env.GOOGLE_DRIVE_IMAGE_INSURANT_COMPANY],
    };
    let media = {
