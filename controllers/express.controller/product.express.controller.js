const {
  ProductExpress,
  validate,
} = require("../../models/express.model/product.express.model");
...
exports.create = async (req, res) => {
  try {
    ...
    if (product) {
      return res
        .status(201)
        .send({status: true, message: "เพิ่มข้อมูลสำเร็จ", result: product});
    } else {
      return res
        .status(400)
    ...
    if (product) {
      return res
        ...
        .send({
          ...
          result: {_id: product._id, shop_id: product.shop_id, ...req.body},
        });
    } else {
      return res
        .status(400)
