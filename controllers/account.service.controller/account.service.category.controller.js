const {
  ...
  validate,
} = require("../../models/account.service.model/account.service.category.model");
...
module.exports.GetAll = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({status: true, message: "ดึงข้อมูลสำเร็จ", data: accountcategory});
  } catch (error) {
    console.error(error);
    return res
      .status(500)
  ...
  try {
    ...
    upload(req, res, async function (err) {
      ...
      if (!req.files) {
        ...
        AccountCategoryModel.findByIdAndUpdate(
          id,
          data,
          ...
          (err, result) => {
            ...
            return res
              ...
              .send({
                ...
                data: {picture: result.picture, name: result.name},
              });
          }
        );
        //end
...
module.exports.delete = async (req, res) => {
  try {
    const id = req.params.id;
    AccountCategoryModel.findByIdAndDelete(
      id,
      {returnOriginal: true},
      (error, result) => {
        if (error) {
