            ...
            async (err, partner_decoded) => {
              if (err) {
                ...
                const productgraphic = await ProductGraphic.findOne({
                  _id: graphicpackage.product_graphic_id,
                });
                if (!productgraphic) {
                ...
                if (productgraphic.detail === "ราคาต่อตารางเมตร") {
                  if (
                    (req.body.product_detail[0].width / 100) *
                      (req.body.product_detail[0].hight / 100) <
                    1
                  ) {
                    totalCost +=
                      (graphicpackage.cost_NBA + graphicpackage.profit_NBA) *
                      req.body.product_detail[0].quantity;
                  } else {
                    totalCost +=
                      (graphicpackage.cost_NBA + graphicpackage.profit_NBA) *
                      ((req.body.product_detail[0].width / 100) *
                ...
                const freight =
                  productgraphic.detail === "ราคาต่อตารางเมตร"
                    ? graphicpackage.freight + calculatefreight
                    : graphicpackage.freight;
                totalPriceWithoutFreight +=
                  pricecalculate * req.body.product_detail[0].quantity;
                const totalPriceWithFreight =
                  totalPriceWithoutFreight + freight;
              ...
              if (partner.partner_wallet < graphicpackage.price) {
                ...
                for (let item of req.body.product_detail) {
                  ...
                  if (container) {
                    ...
                    if (productgraphic) {
                      const plateformprofit =
                        container.price -
                        (container.profit_NBA + container.cost_NBA);
                      ...
                      if (productgraphic.detail === "ราคาต่อตารางเมตร") {
                        if ((item.width / 100) * (item.hight / 100) < 1) {
                          totalCost +=
                            (container.cost_NBA + container.profit_NBA) *
                            item.quantity;
                        } else {
                          totalCost +=
                            (container.cost_NBA + container.profit_NBA) *
                      ...
                      const freight =
                        productgraphic.detail === "ราคาต่อตารางเมตร"
                          ? container.freight + calculatefreight
                          : container.freight;
                      ...
                      return res
                        .status(403)
                        .send({message: "ไม่พบข้อมูลสินค้า"});
                    }
                  }
                }
