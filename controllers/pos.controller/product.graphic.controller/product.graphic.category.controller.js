...
module.exports.GetCategoryById = async (req, res) => {
  try {
    ...
    if (!category) {
      ...
      return res
        .status(200)
        .send({status: true, message: "ถึงข้อมูลสำเร็จ", data: category});
    }
  } catch (error) {
    console.error(error);
    res
  ...
  try {
    ...
    upload(req, res, async function (err) {
      ...
      if (!req.files) {
        ...
        ProductGraphicCategory.findByIdAndUpdate(
          id,
          data,
          ...
          (err, result) => {
            ...
            return res
              ...
              .send({
                ...
                data: {id: result.img_url, name: result.name},
              });
          }
        );
...
module.exports.deleteCategory = async (req, res) => {
  try {
    const id = req.params.id;
    ProductGraphicCategory.findByIdAndDelete(
      id,
      {returnOriginal: true},
      (error, result) => {
        if (error) {
