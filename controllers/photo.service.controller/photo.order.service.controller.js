...
module.exports.order = async (req, res) => {
  try {
    ...
    // }
    const photopackage = await PhotoPackage.findOne({
          ...
          jwt.verify(
            ...
            async (err, partner_decoded) => {
              if (err) {
                ...
                const totalprice =
                  photopackage.price * req.body.product_detail[0].quantity;
                const change = req.body.moneyreceive - totalprice;
                const totalplateformprofit =
                  photopackage.plateformprofit *
                    ...
                    if (data) {
                      ...
                      commission.save((error, data) => {
                        ...
                        const wallethistory = {
                          ...
                          partner_id: "platform",
                          orderid: order._id,
                          name: `รายการสั่งซื้อ Facebook Service ใบเสร็จเลขที่ ${order.receiptnumber}`,
                          type: "เงินออก",
            ...
            if (!findshop) {
              ...
              if (partner.partner_wallet < photopackage.price) {
                ...
                for (let item of req.body.product_detail) {
                  const container = await PhotoPackage.findOne({
                    _id: item.packageid,
                  });
                  if (container) {
                    orders.push({
                ...
                const totalplateformprofit = orders.reduce(
                  (accumulator, currentValue) =>
                    accumulator +
                    currentValue.plateformprofit * currentValue.quantity,
                  0
                );
