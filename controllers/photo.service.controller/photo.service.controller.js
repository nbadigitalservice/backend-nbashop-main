const { PhotoPackage, validate } = require('../../models/photo.model/photo.package.model')
const fs = require('fs')
  ...
  try {
    ...
    upload(req, res, async function (err) {
      ...
      if (!req.files) {
        ...
        const photoservicepackage = new PhotoPackage(data);
        photoservicepackage.save(error => {
          if (error) {
...
//get All photoservice
module.exports.GetAll = async (req, res) => {
  try {
    const photoservice = await PhotoPackage.find();
    return res.status(200).send({ status: true, message: 'ดึงข้อมูลสำเร็จ', data: photoservice })
...
module.exports.update = async (req, res) => {
  try {
    ...
    const packageUpdate = await PhotoPackage.findById(id)
    ...
    upload(req, res, async function (err) {
      ...
      if (!req.files) {
        ...
        PhotoPackage.findByIdAndUpdate(id, data, { returnDocument: 'after' }, (err, result) => {
          if (err) {
...
module.exports.delete = async (req, res) => {
  try {
    const id = req.params.id;
    PhotoPackage.findByIdAndDelete(id, { returnOriginal: true }, (error, result) => {
      if (error) {
