...
const {
  OrderCanceled,
} = require("../../models/order.service.model/order.canceled.model");
const {
  OrderDeliverModel,
} = require("../../models/order.service.model/order.deliver.model");
const {WalletHistory} = require("../../models/wallet.history.model");
const {
  AccountPackageModel,
} = require("../../models/account.service.model/account.service.package.model");
const {
  ActPackageModel,
...
} = require("../../models/insurance.model/insurance.package.model");
const {
  ItsupportPackage,
} = require("../../models/itsupport.model/itsupport.package.model");
const {
  WebsitePackageModel,
} = require("../../models/website.package.model/website.package.model");
const {
  ProductGraphicPrice,
...
module.exports.GetAll = async (req, res) => {
  try {
    ...
    const orderiwithdeliver = await OrderServiceModel.aggregate(
      pipeline
    ).exec();
  ...
  try {
    ...
    if (order.partnername === "shop") {
      ...
      const payload = cryptoJs.AES.encrypt(
        JSON.stringify(canceledOrder),
        process.env.API_GIVE_COMMISSION
      ).toString();
      const request = {
        method: "post",
        headers: {
    ...
    return res
      .status(500)
      .send({message: "มีบางอย่างผิดพลาด", error: error.message});
  }
};
...
module.exports.submit = async (req, res) => {
  try {
    ...
    order.save();
    return res.status(200).send({message: "ดำเนินการเสร็จสิ้น", data: order});
  } catch (error) {
    console.error(error);
    return res
      .status(500)
...
module.exports.GetAllCanceledOrder = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({status: true, message: "ดึงข้อมูลสำเร็จ", data: ordercanceled});
  } catch (error) {
    console.error(error);
    return res
      .status(500)
