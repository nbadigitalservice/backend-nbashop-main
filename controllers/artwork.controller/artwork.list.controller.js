const {
  ArtworkList,
  validate,
} = require("../../models/artwork.model/artwork.list.model");
...
exports.create = async (req, res) => {
  try {
    ...
    if (checklist) {
      return res
        .status(400)
        .send({message: "มีรายการบริการนี้แล้ว", status: false});
    }
    ...
    if (artwork) {
      ...
      return res
        .status(201)
        .send({message: "เพิ่มข้อมูลสำเร็จ", status: true, result: artwork});
    }
  } catch (err) {
    console.log(err);
...
exports.update = async (req, res) => {
  try {
    ...
    if (artwork) {
      return res
        .status(200)
        .send({message: "แก้ไขข้อมูลสำเร็จ", status: true, result: artwork});
    } else {
      return res
        .status(400)
